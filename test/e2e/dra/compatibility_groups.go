...
var _ = framework.SIGDescribe("node")(framework.WithLabel("DRA"), func() {
	...
	f.Context("device compatibility groups", func() {
		f.Context("incompatible devices", func() {
			...
			f.It("leave a pod requesting an incompatible device unschedulable", f.WithFeatureGate(features.DRADeviceCompatibilityGroups), func(ctx context.Context) {
				tCtx := f.TContext(ctx)
		...
		f.Context("compatible devices", func() {
			...
			f.It("schedule two pods that share a compatibility group", f.WithFeatureGate(features.DRADeviceCompatibilityGroups), func(ctx context.Context) {
				tCtx := f.TContext(ctx)
